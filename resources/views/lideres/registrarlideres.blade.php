...
<body class="bg-light">
    <div class="container my-5 p-4 bg-white rounded shadow-sm">
        <h1 class="mb-4 text-center">Formulario</h1>
        ...
        <form action="{{ route('lideres.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="comunidad" id="comunidad" class="form-select" required>
                    ...
                    @foreach ($direcciones as $direccion)
                        <option value="{{ $direccion->comunidad }}" {{ old('comunidad') == $direccion->comunidad ? 'selected' : '' }}>
                            {{ $direccion->comunidad }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="sector" id="sector" class="form-select" required>
                    ...
                    @foreach ($direcciones as $direccion)
                        <option value="{{ $direccion->sector }}" {{ old('sector') == $direccion->sector ? 'selected' : '' }}>
                            {{ $direccion->sector }}
                        </option>
                    @endforeach
                </select>
